...
A vault page may link to a repo-managed asset, but the asset itself is not synced from anywhere: it is committed to the repo and replaced when it changes. Most are hand-replaced; the resume PDF is generated by the public [resume-engine](https://github.com/joeseverino/resume-engine) pipeline from the same canonical that drives the `/resume/` page, which lives in a second private vault (`Career/resume.md`) and is synced with per-line surface markers applied and contact frontmatter whitelisted away. See [Architecture §11 Asset Organization](./Architecture.md#11-asset-organization) for the full convention (which bucket holds what, and the difference between vault-synced and repo-managed assets).
