...
## May 2026 Performance and Security Audit

On May 24, 2026, a comprehensive audit was conducted to compare the production
Astro site against the legacy WordPress origin. This audit measured asset
weight, server response times, security headers, and codebase scale.
...
| --- | ---: | ---: |
| Requests observed | 19 | 13 |
| Transfer observed | 1.16 MB | 476 KB |
| Dominant payload | PNG screenshots | AVIF screenshots |
| Largest observed image | ~600 KB PNG | 89 KB AVIF |
| First-party failures | 0 | 0 |

That is approximately a 58.9% transfer reduction for the observed live load:

```text
(1.16 MB - 476 KB) / 1.16 MB = 58.9%
```

The historical HAR benchmark (recorded below) represents a 92.7% reduction
against an uncached, full-resolution legacy load. This live comparison—validated during the May 24, 2026 audit—shows that even with caching and basic server-side optimizations on the WordPress side, the Astro pipeline provides a consistent ~60% reduction in total weight.
...
### Case Study: Homepage (Audit Date: 5/24/26)
...
| Measurement | Legacy WordPress | Current Astro |
| --- | ---: | ---: |
| Requests observed | 25 | 16 |
| Transfer observed | 1.15 MB | 610 KB |
| Time to First Byte (TTFB) | ~0.38 s | **~0.12 s** |
| First Contentful Paint | 2.7 s | **0.9 s** |
| Accessibility Score | 96 | **100** |

The Astro homepage delivers a significantly lighter payload while achieving a
100% accessibility score, reinforcing the move toward a leaner, more inclusive
technical presence.

### Codebase Profile (Audit Date: 5/24/26)
...
distributed across a database, plugins, and a theme—the current site is fully
contained within ~4,100 lines of handwritten source code.
...
### Server Response and Security (Audit Date: 5/24/26)
...
| --- | ---: | ---: |
| Time to First Byte (TTFB) | ~1.80 s | **~0.13 s** |
| Script Security | `'unsafe-inline'` required | **Nonce-based CSP** |
| Origin Surface | Public PHP/MySQL | **Static Edge** |

The 92% reduction in TTFB is a direct result of moving from dynamic PHP
rendering to static edge delivery. Additionally, the move to a [nonce-based Content Security Policy](./Architecture.md#9-edge-security) significantly reduces the site's script-injection risk, a level of protection that was difficult to maintain with WordPress's dependency on inline scripts.
...
The current Astro Lighthouse run on May 24, 2026 reported:
