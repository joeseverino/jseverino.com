...
## 2. The Content Sync Engine
The sync engine ([`bin/sync-content.mjs`](../bin/sync-content.mjs)) is an asynchronous Node.js pipeline responsible for the "Sanitization & Optimization" pass. It ensures that the public repository contains only the data necessary for the build.
...
The "magic" of the site's rich content lies in a custom transformation layer built on top of `markdown-it`, implemented in [`src/lib/content.ts`](../src/lib/content.ts).
...
*   **Striped Tables**: The `table_open` rule is overridden to wrap every Markdown table in a `<figure class="table-figure table-figure--striped">`. This ensures consistent styling and mobile horizontal scrolling.
*   **Intelligent Figures**: Standard Markdown images are promoted to `<figure>` elements if alt text is present, supporting advanced syntax like `![alt|width|nocap]` for fine-grained layout control.
...
### Nonce-Based CSP
The site enforces a **Strict Content Security Policy** defined in [`public/_headers`](../public/_headers) using a nonce-injection middleware ([`functions/_middleware.ts`](../functions/_middleware.ts)):
...
*   **Headers**: The `Content-Security-Policy` header is emitted with the matching nonce, blocking all XSS and unauthorized script injection.

### Secure Dynamic Boundary
The contact form ([`src/components/ContactForm.astro`](../src/components/ContactForm.astro)) is the only non-static surface. It is protected by a Cloudflare Pages Function ([`functions/api/contact.ts`](../functions/api/contact.ts)) that implements:
...
The site uses a structured taxonomy system to manage technical metadata consistently across all writeups.

*   **Single Source of Truth**: [`src/content/technology-groups.md`](../src/content/technology-groups.md) serves as the canonical mapping of slugs to human-readable labels.
*   **Automatic Resolution**: The transformation engine ([`src/lib/content.ts`](../src/lib/content.ts)) resolves tag slugs into their mapped labels at build-time. This ensures that a tag like `cloudflare-pages` is always rendered as "Cloudflare Pages" without manual overrides in each writeup.
*   **Stale Slug Guard**: The build process emits a console warning if a writeup references a technology slug that isn't defined in the taxonomy file, preventing "unknown" tags from shipping to production.

---

## 7. Local Toolchain & Workflow
...
*   [`bin/audit-assets.mjs`](../bin/audit-assets.mjs): Automatically fails the build if any image exceeds a 1.5MB weight threshold.
*   [`bin/csp-hashes.mjs`](../bin/csp-hashes.mjs): Generates the `sha256` hashes for all inline scripts to be pasted into the production headers.
