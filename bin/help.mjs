...
const GROUPS = [
  ...
  },
  {
    ...
    items: {
      ...
      'test:edge': 'Serve the build through the Cloudflare runtime and assert headers, CSP nonces, and the functions',
      'edge:serve': 'Serve the build through wrangler pages dev (middleware + functions active) for manual checks',
  ...
  },
  {
    ...
    items: {
      ...
      'check:css-vars': 'Individual audit (gates run it via tests/audits/registry.mjs)',
      ...
      'check:repo-policy': 'Individual audit (gates run it via tests/audits/registry.mjs)',
