...
| --- | --- | --- |
| [`ci`](./.github/workflows/ci.yml) | One pipeline drawn as a dependency graph. `gate` (repository policy, documentation integrity, stylesheet lint) fans out to `build` (the registry publish gate `publish:check --no-sync` in a clean container, plus a CycloneDX SBOM), `e2e` (cross-browser Playwright), and `visual` (macOS Chromium baselines). On a push to `main` those converge on `verify`, which runs `deploy:verify` against production: required checks and the Cloudflare Pages build, then security headers, per-request CSP nonces, sitemap routes, a real 404, the contact form's Turnstile gate, and `security.txt` parity. On a pull request, `report` gathers every job's summary into one comment that updates in place on each push. | Green graph on the committed tree, SBOM artifact, and a summary on every job: gate audit results, the publish gate's step table, per-browser Playwright results, and the per-check production table from `verify`; a failed probe opens an issue that closes on the next clean run. |
