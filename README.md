...
| --- | --- | --- |
| [`ci`](./.github/workflows/ci.yml) | One pipeline drawn as a dependency graph. `gate` (source integrity, repository policy, documentation integrity, stylesheet lint) fans out to `build` (the registry publish gate `publish:check --no-sync` in a clean container, plus a CycloneDX SBOM) and the `playwright` matrix: `e2e` (cross-browser), `visual` (macOS Chromium baselines), and `edge`, which serves the same build through the Cloudflare runtime with `wrangler pages dev` and asserts what only that runtime produces: the per-request CSP nonce on every script tag, the `_headers` security and cache rules, a real 404, the contact function's refusals, and byte-exact `security.txt`, all before anything deploys. `deploy` waits for Cloudflare Pages to finish deploying the same commit. On a pull request, `report` gathers every job's summary into one comment that updates in place on each push. | Green graph on the committed tree, SBOM artifact, and a summary on every job: gate audit results, the publish gate's step table, per-suite Playwright results, and the Cloudflare Pages deployment. |
