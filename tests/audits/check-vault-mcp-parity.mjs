...
// the fields the vault MCP's `update_writeup_frontmatter` tool accepts, what
...
// interactive editor silently can't touch.
...
for (const gap of [zodGap, mcpGap, tuiGap]) {
...
console.log(`ok       schema/Zod/MCP/TUI agree on writeup fields: ${sharedFields}`);
